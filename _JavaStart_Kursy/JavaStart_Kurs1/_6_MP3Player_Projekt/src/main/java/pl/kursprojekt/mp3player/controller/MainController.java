...
import pl.kursprojekt.mp3player.mp3.Mp3Parser;
import pl.kursprojekt.mp3player.mp3.Mp3Song;
import pl.kursprojekt.mp3player.player.Mp3Player;
...
public class MainController {
    ...
    private void configureProgressBar() {
        ...
        //ustawienie d³ugoœci suwaka postepu
        player.getMediaPlayer().setOnReady(() -> progressSlider.setMax(player.getLoadedSongLength()));
        //zmiana czasu w odtwarzaczu automatycznie bêdzie aktualizwa³a suwak
        player.getMediaPlayer().currentTimeProperty().addListener((arg, oldVal, newVal) ->
                progressSlider.setValue(newVal.toSeconds()));
        //przesuniêcie suwaka spowoduje przewiniêcie piosenki do wskazanego miejsca
        progressSlider.valueProperty().addListener((observabele, oldValue, newValue) -> {
    ...
    private void configureMenu() {
        ...
        openFile.setOnAction(event -> {
            ...
            try {
                ...
                showMessage("Za³adowano plik " + file.getName());
            } catch (Exception e) {
                showMessage("Nie mo¿na otworzyæ pliku " + file.getName());
            }
        });
        ...
        openDir.setOnAction(event -> {
            ...
            try {
                ...
                showMessage("Wyst¹pi³ b³¹d podczas odczytu folderu.");
            }
        });
    }
