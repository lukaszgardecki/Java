package com.mojastrona.generator;
...
public class NumbersGeneratorController {
    ...
    @GetMapping("/random")
    @ResponseBody
    String getRandomNumber(@RequestParam(required = false) Integer from, @RequestParam(required = false) Integer to) {
    ...
    @ResponseBody
    String getEvenNumbers(@RequestParam(required = false) Integer from, @RequestParam(required = false) Integer to) {
        if (incorrectParams(from, to)) {
            return INCORRECT_PARAMS_MESSAGE;
        }
        int lowerBound = getLowerBound(from, to);
    ...
    @ResponseBody
    String getOddNumbers(@RequestParam(required = false) Integer from, @RequestParam(required = false) Integer to) {
        if (incorrectParams(from, to)) {
            return INCORRECT_PARAMS_MESSAGE;
        }
        int lowerBound = getLowerBound(from, to);
