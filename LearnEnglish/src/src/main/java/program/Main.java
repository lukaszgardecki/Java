...
public class Main {
    ...
    static final MemoBox memoBox = new MemoBox(5, 10, 15, 20, 25);
    ...
    public static String[] loadWord1() {
        String[] word;
        ...
        if (isFull_Group1) {
            word = getFirstWordFromGroup1(1);
        } else if (isFull_Group2) {
            word = getFirstWordFromGroup1(2);
        } else if (isFull_Group3) {
            word = getFirstWordFromGroup1(3);
        } else if (isFull_Group4) {
            word = getFirstWordFromGroup1(4);
        } else if (isFull_Group5) {
            word = getFirstWordFromGroup1(5);
        } else if (areWordsInGroupZero) {
            word = getFirstWordFromGroup1(0);
        } else if (isMapEmpty) {
            word = displayEmptyBaseMessage1();
        } else {
            word = getTheOldestWord1();
        }
        setUnderscores(word);
        return word;
    }

    private static String[] displayEmptyBaseMessage1() {
        return new String[]{"19940418200515", "0", "0", "0", "brak fiszek", "brak fiszek"};
    }

    private static String[] getFirstWordFromGroup1(int group) {
        //return map.get(group).getFirst();
        return new String[]{};
    }

    public static String[] getTheOldestWord1() {
