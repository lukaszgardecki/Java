...
public class PlikDanych4 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        long poz;
        try( RandomAccessFile plikRaf = new RandomAccessFile( plikBin, "rw");)
        {
            while( true ){
                towar   = plikRaf.readUTF();
                lSztuk  = plikRaf.readInt();
                poz     = plikRaf.getFilePointer();
                cena    = plikRaf.readDouble();
                if( lSztuk > 100 ){
                    plikRaf.seek(poz);
                    plikRaf.writeDouble( cena * 0.9 );
                }
            }
        }
